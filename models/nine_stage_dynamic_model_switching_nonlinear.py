...
def build_model(mode_transfer=False):
    ...
    # Set
    model.stage = Set(initialize=[1, 2, 3, 4, 5, 6, 7, 8, 9])
    ...
    if mode_transfer:
        ...
        model.transfer_stage1 = Set(initialize=[2, 3, 4, 5, 6, 7, 8, 9])
        model.transfer_stage2 = Set(initialize=[2, 3, 4, 5, 6, 7, 8, 9, 10])
        ...
        def _mode_transfer_rule4(model):
            return model.mode_transfer[10, 1].implies(
                lnot(
                    lor(
        ...
        def _mode_transfer_rule5(model):
            return model.mode_transfer[10, 2].implies(
                lnot(
                    lor(
