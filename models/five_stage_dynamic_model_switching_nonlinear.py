...
def build_model(mode_transfer=False):
    ...
    # Set
    model.stage = Set(initialize=[1, 2, 3, 4, 5])
    ...
    if mode_transfer:
        ...
        model.transfer_stage1 = Set(initialize=[2, 3, 4, 5])
        model.transfer_stage2 = Set(initialize=[2, 3, 4, 5, 6])
        ...
        def _mode_transfer_rule4(model):
            return model.mode_transfer[6, 1].implies(
                lnot(
                    lor(
        ...
        def _mode_transfer_rule5(model):
            return model.mode_transfer[6, 2].implies(
                lnot(
                    lor(
    ...
    model.obj = Objective(
        expr=-(model.intx1 + model.intx2 + model.intx3 + model.intx4 + model.intx5),
        sense=minimize,
    )
    return model
